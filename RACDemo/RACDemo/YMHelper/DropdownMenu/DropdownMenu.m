...
- (void)selectedFirstValues:(NSArray *)values withTitle:(NSString *)title {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(dropdownSelectedBtnIndex:leftIndex:rightIndex:text:)]) {
        [_delegate dropdownSelectedBtnIndex:_btnSelectedIndex leftIndex:[values[0] integerValue] rightIndex:[values[1] integerValue] text:title];
    }
